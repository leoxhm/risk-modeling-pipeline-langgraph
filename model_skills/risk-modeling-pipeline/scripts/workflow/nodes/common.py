...
def normalize_engine_root(value: str | Path) -> Path:
    """Normalize the installed Skill root accepted by model-generated commands.

    The canonical layout is ``<engine-root>/risk-modeling-pipeline``. Older
    prompts sometimes passed that nested directory itself as ``--engine-root``;
    accepting it here keeps assets and templates resolvable without relying on
    the caller to guess the layout correctly.
    """
    root = Path(value).expanduser().resolve()
    if root.name == "risk-modeling-pipeline" and (root / "scripts").is_dir():
        return root.parent
    return root
...
def resolve_engine_assets(args: Any) -> Path:
    ...
    if getattr(args, "engine_root", None):
        root = normalize_engine_root(args.engine_root)
    else:
...
def ensure_workspace_configs(
    ...
    if nodes:
        nodes.add("data-read")
    if nodes & {"sample-diagnosis", "feature-processing", "model-config", "training-tuning", "model-review", "report-delivery"}:
        nodes.add("eda-analysis")
    if nodes & {"model-config", "training-tuning", "model-review", "report-delivery"}:
        nodes.add("feature-processing")
    if nodes & {"model-config", "training-tuning", "model-review", "report-delivery"}:
...
class NodeContext:
    ...
    def from_args(
        ...
        node_id: str,
        *,
        skip_config_validation: set[str] | None = None,
    ) -> "NodeContext":
...
def write_node_summary(
    ...
    *,
    ...
    """Persist a short deterministic node conclusion for the host UI.

    The summary is generated by the workflow code, not by an LLM.  A host can
    display this one small JSON artifact after each node without reading the
    full HTML/CSV report or asking the model to re-summarize it.
    """
    path = output_dir / "node_summary.json"
...
def approval_path(context: NodeContext, node_id: str, *, for_write: bool = False) -> Path:
    """Return the dedicated approval path, with read-only legacy fallback."""
    current = context.approval_dir / f"{node_id}.approval.json"
    if for_write or current.is_file():
        return current
    legacy = context.node_config_dir / f"{node_id}.approval.json"
